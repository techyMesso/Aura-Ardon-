...
- Password-based admin login tied to a single configured admin email
...
2. Use the existing user `aura.ardoncompany@gmail.com` (do not create a second admin)
3. Open that user and set a password (at least 8 characters)
4. Confirm the user can sign in (auto-confirm if needed)
5. Set `ADMIN_EMAIL=aura.ardoncompany@gmail.com` in your hosting environment

The login form is locked to that admin email. Only the password is entered.

Do **not** put the admin password in environment variables. It lives only on the Supabase Auth user.

Set `NEXT_PUBLIC_SITE_URL` to your deployed domain in production, for example `https://your-app.vercel.app`.
