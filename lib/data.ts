...
/**
 * List featured products for homepage merchandising.
 ...
 */
async function listProductsByCategoryInner(
  categorySlug: string
): Promise<Product[]> {
  if (!hasPublicSupabaseEnv()) return [];
  ...
  try {
    ...
    const categoryRecord = cat as { id: string };
...
export function listProductsByCategory(categorySlug: string): Promise<Product[]> {
  return unstable_cache(
    () => listProductsByCategoryInner(categorySlug),
    ["products-by-category", categorySlug],
    { tags: ["products", "categories"], revalidate: 120 }
  )();
}
 ...
 */
async function getProductBySlugInner(
  slug: string
...
export type CategoryRepresentative = {
  category_id: string;
  image_url: string;
};

async function listCategoryRepresentativesInner(): Promise<CategoryRepresentative[]> {
  if (!hasPublicSupabaseEnv()) return [];
  ...
  try {
    ...
    if (error) {
      logger.error("Failed to list category representative images", {
        error: error.message
      });
      return [];
    }
    ...
    for (const record of (data ?? []) as Array<{ category_id: string | null; images: string[] | null }>) {
      const image = record.images
        ?.map(getSafeCatalogImageUrl)
        .find((value): value is string => Boolean(value));
      if (record.category_id && image && !representatives.has(record.category_id)) {
        representatives.set(record.category_id, image);
      }
    }

    return [...representatives].map(([category_id, image_url]) => ({
      category_id,
      image_url
    }));
  } catch (err) {
    logger.error("Category representative images unavailable", {
      error: err instanceof Error ? err.message : String(err)
    });
    return [];
  }
...
export const listCategoryRepresentatives = unstable_cache(
  listCategoryRepresentativesInner,
